...
class FirestoreUserDataService {
  ...
  Future<void> ensureCurrentUserDocument() async {
    ...
    try {
      await _userDocument(user.uid).set(<String, dynamic>{
        ...
        'likedSongs': const <String>[],
        'dislikedSongs': const <String>[],
  ...
  Future<void> setLikedSong({
    ...
    if (_useRestApiOnWindows) {
      try {
        ...
        final Set<String> likedSongIds = Set<String>.from(existing.likedSongIds);
  ...
  Future<List<UserPlaylist>> _loadPlaylistsRest(User user) async {
    ...
    final List<UserPlaylist> playlists = documents
        .map(_playlistFromRestDocument)
        .toList(growable: false)
      ..sort(_sortPlaylists);
    return playlists;
  }
  ...
  Future<List<Map<String, dynamic>>> _listRestDocuments(String path) async {
    ...
    return rawDocuments.whereType<Map<String, dynamic>>().toList(growable: false);
  }
  ...
  UserPlaylist _playlistFromRestDocument(Map<String, dynamic> document) {
    ...
    return UserPlaylist(
      ...
      songIds: _readRestStringArrayField(fields['songIds']).toList(growable: false),
  ...
  DateTime _readRestDateTimeField(Object? value) {
    ...
    final String raw =
        (value['timestampValue'] as String? ?? value['stringValue'] as String? ?? '')
            .trim();
